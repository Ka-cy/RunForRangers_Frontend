...
export class ReceiveStockComponent implements OnInit {
  ...
  // Define image options for each product (mimic AddInventoryComponent)
...
userId:any=JSON.parse(sessionStorage.getItem('adminData')||'{}').userId;
  ...
  checkDataLoaded() {
    // Only set isLoading to false when all data is loaded
  ...
  updateImageUrl() {
    // This method can be used if additional logic is needed when imageUrl changes
  }
  ...
  onSubmit(form: NgForm) {
    ...
    this.inventoryService.receiveStock(payload,this.userId).subscribe({
      next: () => {
        this.isLoading = false;
        this.snackBar.open('✅ Stock received successfully!', 'Close', {
          duration: 3000,
      ...
      error: (err: any) => {
        this.isLoading = false;
        console.error(err);
        this.snackBar.open('❌ Error receiving stock.', 'Close', {
          duration: 3000,
          panelClass: ['error-snackbar']
        });
      }
