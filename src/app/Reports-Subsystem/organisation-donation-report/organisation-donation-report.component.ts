...
@Component({
  selector: 'app-organisation-donation-report',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './organisation-donation-report.component.html',
  styleUrls: ['./organisation-donation-report.component.css']
})
export class OrganisationDonationReportComponent implements OnInit {
  ...
  fetchDonations(): void {
    ...
    this.donationService.getOrgDonationsByDateRange(start, end).subscribe({
      next: (data) => {
        this.donations = data;
        this.updateChart();
      },
  ...
  generatePdf(): void {
    try {
      ...
      doc.text('Organisation Donation Report', 105, 20, { align: 'center' });
      ...
      // Save PDF
      doc.save(`organisation_donation_report_${new Date().toISOString().split('T')[0]}.pdf`);
